...
const Filtermoto = (loaixe, gia, pagenumber,city) => async (dispatch) => {
    try {
        ...
        if (loaixe == 'ALL' && gia == 'ALL') {
            const { data } = await axios.get("http://localhost:4000/api/products/?city="+city+"&_page=" + pagenumber + "&_limit=6");
            dispatch({
            ...
            if (loaixe == 'ALL') {
                const { data } = await axios.get("http://localhost:4000/api/products/?_sort=price&_order=" + gia + "&city="+city+"&_page=" + pagenumber + "&_limit=6")
                dispatch({
                    type: FILTER_MOTO_GIA_SUCCESS, payload: data.data
                });
            }
            else {
                const { data } = await axios.get("http://localhost:4000/api/products/?brand=" + loaixe + "&city="+city+"&_page=" + pagenumber + "&_limit=6")
                dispatch({
            ...
            const { data } = await axios.get("http://localhost:4000/api/products/?_sort=price&_order=" + gia + "&brand=" + loaixe + "&city="+city+"&_page=" + pagenumber + "&_limit=6")
            dispatch({
...
const FilterFood = (city,brand,rating,price,page) => async (dispatch) => {
    try {
        ...
        if (rating == 'ALL' && price == 'ALL') {
            ...
            if (rating == 'ALL') {
                const { data } = await axios.get("http://localhost:4000/api/foods/?sort=" + price + "&city="+city+"&brand="+brand+"&_page=" + page + "&_limit=4")
                dispatch({
                    type: FILTER_FOOD_GIA_SUCCESS, payload: data.data
                });
            }
            else {
                const { data } = await axios.get("http://localhost:4000/api/foods/?brand=" + brand + "&city="+city+"&rating="+rating+"&_page=" + page + "&_limit=4")
                dispatch({
            ...
            const { data } = await axios.get("http://localhost:4000/api/foods/?sort=" + price + "&brand=" + brand + "&city="+city+"&rating="+rating+"&_page=" + page + "&_limit=4")
            dispatch({
...
const SearchName =(searchtext,pagenumber,city)=> async (dispatch)=>{
    try{
        ...
        const { data } = await axios.get("http://localhost:4000/api/products/?name="+searchtext+"&city="+city+"&_page=" + pagenumber + "&_limit=6");
            dispatch({
